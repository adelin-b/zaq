defmodule Zaq.Channels.Api do
  ...
  def handle_event(%Event{request: request} = event, :materialize_record, _context)
    ...
    %{event | response: communication_bridge.materialize_record(request, opts)}
  end
  ...
  def handle_event(
      ...
      when is_map(params) do
    data_source_module = Keyword.get(event.opts, :data_source_bridge_module, DataSourceBridge)
    %{event | response: data_source_module.watch_item(provider, params)}
  end
  ...
  def handle_event(
      ...
      when is_map(params) do
    data_source_module = Keyword.get(event.opts, :data_source_bridge_module, DataSourceBridge)
    %{event | response: data_source_module.unwatch_item(provider, params)}
  end
